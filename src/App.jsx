...
export default function App() {
  ...
  // Auto-migrate DOTS to CROW (one-time migration)
  useEffect(() => {
    const migrateUserDotsToCrow = async () => {
      if (!user || !userData) return;

      const OLD_TICKER = 'DOTS';
      const NEW_TICKER = 'CROW';

      // Check if user has any DOTS data that needs migration
      const hasDots =
        (userData.holdings && userData.holdings[OLD_TICKER]) ||
        (userData.shorts && userData.shorts[OLD_TICKER]) ||
        (userData.costBasis && userData.costBasis[OLD_TICKER]) ||
        (userData.lowestWhileHolding && userData.lowestWhileHolding[OLD_TICKER]) ||
        (userData.watchlist && userData.watchlist.includes(OLD_TICKER));

      if (!hasDots) return; // No migration needed

      console.log('Migrating user DOTS → CROW data...');
      ...
      try {
        ...
        // Migrate holdings
        if (userData.holdings && userData.holdings[OLD_TICKER]) {
          updates[`holdings.${NEW_TICKER}`] = userData.holdings[OLD_TICKER];
          updates[`holdings.${OLD_TICKER}`] = null;
        }

        // Migrate shorts
        if (userData.shorts && userData.shorts[OLD_TICKER]) {
          updates[`shorts.${NEW_TICKER}`] = userData.shorts[OLD_TICKER];
          updates[`shorts.${OLD_TICKER}`] = null;
        }

        // Migrate cost basis
        if (userData.costBasis && userData.costBasis[OLD_TICKER]) {
          updates[`costBasis.${NEW_TICKER}`] = userData.costBasis[OLD_TICKER];
          updates[`costBasis.${OLD_TICKER}`] = null;
        }

        // Migrate lowestWhileHolding
        if (userData.lowestWhileHolding && userData.lowestWhileHolding[OLD_TICKER]) {
          updates[`lowestWhileHolding.${NEW_TICKER}`] = userData.lowestWhileHolding[OLD_TICKER];
          updates[`lowestWhileHolding.${OLD_TICKER}`] = null;
        }

        // Migrate watchlist
        if (userData.watchlist && Array.isArray(userData.watchlist) && userData.watchlist.includes(OLD_TICKER)) {
          const newWatchlist = userData.watchlist.map(t => t === OLD_TICKER ? NEW_TICKER : t);
          updates.watchlist = newWatchlist;
        }

        if (Object.keys(updates).length > 0) {
          await updateDoc(userRef, updates);
          console.log('User DOTS → CROW migration complete');
        }
      } catch (err) {
        console.error('Error migrating DOTS → CROW:', err);
      }
    };

    migrateUserDotsToCrow();
  }, [user, userData]);

  // Auto-migrate market data DOTS to CROW (one-time migration, admin-level)
  useEffect(() => {
    const migrateMarketDotsToCrow = async () => {
      ...
      // Check if market has DOTS data
      const hasDots =
        (marketData.prices && marketData.prices[OLD_TICKER]) ||
        (marketData.priceHistory && marketData.priceHistory[OLD_TICKER]);

      if (!hasDots) return;

      console.log('Admin: Migrating market DOTS → CROW data...');
      ...
      try {
        ...
        // Migrate price
        if (marketData.prices && marketData.prices[OLD_TICKER]) {
        ...
        if (Object.keys(updates).length > 0) {
          ...
          console.log('Market DOTS → CROW migration complete');
        }
      } catch (err) {
        console.error('Error migrating market DOTS → CROW:', err);
      }
    };

    migrateMarketDotsToCrow();
  }, [user, marketData]);
