...
// Shared dividend payout routine. Exported below as both a scheduled function
// and an admin-callable so we can trigger it manually for testing or recovery.
 ...
 *
 * Launch policy: grandfather all existing shares straight into `eligible` (no
 * 10-day wait). Rationale — at launch only ETFs pay dividends, and nobody has
 * been farming ETFs, so there's no exploit risk. For non-ETF stocks (growth
 * tier, 0% payout) the eligible/pending distinction is cosmetic today; putting
 * them in eligible means if/when an individual character is later promoted
 * into the dividend or blue-chip tier, their existing holders are immediately
 * eligible rather than having to re-serve a 10-day wait on holdings they've
 * already been sitting on for weeks.
 *
 * New buys going forward still enter pending with the 10-day wait — only the
 * one-time grandfather pass is immediate.
 *
