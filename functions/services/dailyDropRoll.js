...
// The two tables draw from disjoint tiers, so a ticker normally appears once.
// It can collide when a tier is empty and both draws fall back to the full
// roster — and the award loop writes one holdings key per ticker, so a
// duplicate would silently drop shares. Fold them together instead.
function mergePicks(picks) {
  ...
  for (const pick of picks) {
    ...
    if (pick.group === 'main') existing.group = 'main';
  }
 ...
 *          'bonus' | 'main'` so the Discord embed can show the two tables.
 */
function rollDailyStock(prices, launchedTickers = []) {
  ...
  return { picks: mergePicks([...bonus, ...main]), isJackpot: false };
}
